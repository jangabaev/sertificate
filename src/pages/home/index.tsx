...
export const Dashborad: React.FC = () => {
  const [answers, setAnswers] = useState(Array(dataMock.length).fill("A"));
    ...
    const testCheck=async ()=>{
      try {
        const test={
          ...
          test:["a", "a", "a", "a", "b", "b", "b", "b", "b", "b", "b", "b", "b", "b", "b", "b", "b", "c", "b", "a", "a", "a", "a", "a", "a", "a", "a", "a", "a", "a"]
        }
                ...
                <div key={qIndex} className="mb-6">
                  ...
                  <div className="flex gap-6">
                    {Array(q.options).fill(null).map((_,index) => (
                       <label
                        key={index}
                        className={`flex items-center gap-2 rounded-full font-semibold text-xl bg-gray-200 hover:bg-green-200 cursor-pointer relative w-[40px] h-[40px] border justify-center ${answers[qIndex] === answersResponce(index) && "bg-green-400 hover:bg-green-400"}`}
                      >
                        <input
                          type="radio"
