...
export const Profil = () => {
  ...
  return (
    <main className="min-h-screen bg-[rgb(var(--background))] px-4 pb-24 pt-4 text-[rgb(var(--text))]">
      ...
      {selectedTest && (
        <div
          className="fixed inset-0 z-50 flex items-end bg-black/50 px-3 pb-3 pt-10 sm:items-center sm:justify-center"
          onClick={() => setSelectedTest(null)}
        >
          <section
            className="max-h-[88vh] w-full max-w-xl overflow-hidden rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--surface))] shadow-2xl"
          ...
          >
            ...
            <div className="max-h-[72vh] overflow-y-auto p-4">
              {selectedQuestions.length === 0 ? (
                ...
                <div className="flex flex-col divide-y divide-[rgb(var(--border))] overflow-hidden rounded-xl border border-[rgb(var(--border))]">
                  {selectedQuestions.map((_, index) => {
                    const isCorrect = selectedTest.checks?.[index] === 1;
                    const userAnswer = selectedTest.userAnswers?.[index];
                    const correctAnswer = selectedTest.correctAnswers?.[index];

                    return (
                      <div
                        key={index}
                        className="grid grid-cols-[42px_minmax(0,1fr)_44px] items-center gap-3 bg-[rgb(var(--surface))] px-3 py-3"
                      >
                        <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[rgb(var(--background))] text-sm font-bold text-[rgb(var(--text-muted))]">
                          {index + 1}
                        </span>
                        <div className="min-w-0">
                        ...
                        <span
                          ...
                          title={isCorrect ? "To'g'ri" : "Noto'g'ri"}
                        >
                          {isCorrect ? "✅" : "❌"}
                        </span>
                      </div>
